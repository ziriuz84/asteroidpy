...
def config_canonical(home_dir: str) -> str:
    return os.path.join(home_dir, ".config", cfg.APP_NAME, cfg.CONFIG_FILENAME)


def config_legacy(home_dir: str) -> str:
    return os.path.join(home_dir, cfg.CONFIG_FILENAME)
...
class ConfigurationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.platformdirs_patcher = patch.object(
            platformdirs,
            "user_config_dir",
            side_effect=lambda appname, appauthor=False, **_: os.path.join(
                self.fake_home, ".config", appname
            ),
        )
        self.expanduser_patcher.start()
        self.platformdirs_patcher.start()
    ...
    def test_save_config_writes_file(self):
        ...
        path = config_canonical(self.fake_home)
    ...
    def test_change_obs_altitude_updates_file(self):
        write_config_file(
            config_canonical(self.fake_home), create_minimal_config_text(altitude="0.0")
        )
    ...
    def test_migrates_legacy_config_file(self):
        ...
        conf = self.new_config()
        cfg.load_config(conf)
        self.assertEqual(conf.get("General", "lang"), "it")
        self.assertTrue(os.path.exists(canon))
